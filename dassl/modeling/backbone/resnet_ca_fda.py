...
class ResNet(Backbone):
    ...
    def featuremaps(self, x):
        ...
        x3 = self.layer3(x2)
        x = self.layer4(x3)
        return x, [self.global_avgpool(x1), 
                   self.global_avgpool(x2), 
                   self.global_avgpool(x3), 
                   self.global_avgpool(x)]

    def forward(self, x, out_mid_feat=True):
        f, mid_feat = self.featuremaps(x)
        v = self.global_avgpool(f)
        if out_mid_feat:
            return v.view(v.size(0), -1), mid_feat
        else:
...
def resnet18_ca_fda(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet34_ca_fda(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet50_ca_fda(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet101_ca_fda(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet152_ca_fda(pretrained=True, **kwargs):
