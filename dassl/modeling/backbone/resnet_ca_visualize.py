...
class ResNet(Backbone):
    ...
    def featuremaps(self, x):
        ...
        feat.append(x)
        return x, [out_ca1, out_ca2, out_ca3, out_ca4], feat

    def forward(self, x):
        f, channel_atten, feat_map = self.featuremaps(x)
        v = self.global_avgpool(f)
        return v.view(v.size(0), -1), channel_atten, feat_map
...
def resnet18_ca_visualize(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet34_ca_visualize(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet50_ca_visualize(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet101_ca_visualize(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet152_ca_visualize(pretrained=True, **kwargs):
