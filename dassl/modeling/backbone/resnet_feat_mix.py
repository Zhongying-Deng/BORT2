...
from dassl.utils import mix_amplitude #, cutmix, rand_bbox
...
class CutMix(nn.Module):
    ...
    def forward(self, x, label=None, domain_bs=None):
        ...
        if r <= self.prob and self.cn_mix and False:
            x = channel_mix(x)#
        x_mix, label1, lam, rand_index = feat_content_mix(x, self.mix_beta, label)
        #else:
...
def feat_content_mix(x, beta, label=None, domain_bs=None):
        B, C, W, H = x.size()
...
class ResNet(Backbone):
    ...
    def featuremaps(self, x, label=None, domain_btcsize=None, use_fmix=True):
        ...
        if self.fmix_cfg[0] == 1 and use_fmix:
            out_fmix = self.feat_mix0(x, label=label, domain_bs=domain_btcsize)
            x = out_fmix[0]
        ...
        if self.fmix_cfg[1] == 1 and use_fmix and self.training:
            out_fmix = self.feat_mix1(x, label=label, domain_bs=domain_btcsize)
            x = out_fmix[0]
        x = self.layer2(x)
        if self.fmix_cfg[2] == 1 and use_fmix and self.training:
            out_fmix = self.feat_mix2(x, label=label, domain_bs=domain_btcsize)
            x = out_fmix[0]
        x = self.layer3(x)
        if self.fmix_cfg[3] == 1 and use_fmix and self.training:
            out_fmix = self.feat_mix3(x, label=label, domain_bs=domain_btcsize)
            x = out_fmix[0]
        x = self.layer4(x)
        if self.fmix_cfg[4] == 1 and use_fmix and self.training:
            out_fmix = self.feat_mix4(x, label=label)
            x = out_fmix[0]
        return x, out_fmix

    def forward(self, x, label=None, domain_btcsize=None, use_fmix=True):
        f, fmix = self.featuremaps(x, label=label,
                domain_btcsize=domain_btcsize, use_fmix=use_fmix)
...
def resnet18_fmix(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet34_fmix(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet50_fmix(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet101_fmix(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet152_fmix(pretrained=True, **kwargs):
