...
class CNN(Backbone):
    ...
    def __init__(self):
        ...
        self.conv2_adapt = GlobalMultiScaleAdaptKernelModule(3, 64, 128, m=16, padding=1, stride=2)
    ...
    def forward(self, x, domain_btcsize=None, use_mixstyle=True):
        ...
        x1 = self.conv2(x)
        #x1 = F.relu(x1)
        x2 = self.conv2_adapt(x)
        #x2 = F.relu(x2)
        padding = (2, 1, 2, 1)
        x2 = F.pad(input=x2, pad=padding, mode='constant', value=0)
        x = F.relu(x1 + x2)
...
def cnn_digitsingle_adapt_kernel_ms(**kwargs):
    model = CNN()
