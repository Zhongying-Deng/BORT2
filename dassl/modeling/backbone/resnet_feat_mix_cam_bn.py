...
class DomainSpecificBN(nn.Module):
    def __init__(self, planes, domain_num):
        super().__init__()
        self.domain_num = domain_num
        self.bns = nn. ModuleList(
                [nn.BatchNorm2d(planes)  for _ in range(2)]
                )

    def forward(self, x, domain_btcsize=None, use_dsbn=False):
        if self.training:
            # target domain is always corresponding to the last BN layer in the self.bns
            if domain_btcsize is not None:
                if len(domain_btcsize) == self.domain_num and use_dsbn:
                    x_split = torch.split(x, domain_btcsize, 0)
                    x_src = torch.cat(x_split[:-1], 0)
                    x_tar = x_split[-1]
                    x_src = self.bns[0](x_src)
                    x_tar = self.bns[-1](x_tar)
                    x = torch.cat([x_src, x_tar], 0)
                else:
                    x = self.bns[0](x)
            else:
                x = self.bns[0](x) # no target data in x
        else:
            # when testing, target domain corresponds to the last BN layer in the self.bns
            x = self.bns[-1](x)
        return x
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, domain_num, stride=1, downsample=None):
        super().__init__()
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = DomainSpecificBN(planes, domain_num) #nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = DomainSpecificBN(planes, domain_num) #nn.BatchNorm2d(planes)
    ...
    def forward(self, x, domain_btcsize=None, use_dsbn=False):
        ...
        if self.downsample is not None:
            #import pdb
            #pdb.set_trace()
            residual = self.downsample._modules['0'](x)
            residual = self.downsample._modules['1'](
                    residual,
                    domain_btcsize=domain_btcsize,
                    use_dsbn=use_dsbn
                    )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, domain_num, stride=1, downsample=None):
        ...
        self.bn3 = DomainSpecificBN(planes * self.expansion, domain_num) #nn.BatchNorm2d(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class MakeDomainSpecificLayer(nn.Module):
    def __init__(self, block, inplanes, planes, blocks, stride=1, domain_num=None):
        super().__init__()
        downsample = None
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                DomainSpecificBN(planes * block.expansion, domain_num)
                #nn.BatchNorm2d(planes * block.expansion),
            )
        self.layers = nn.ModuleList()
        self.layers.append(block(inplanes, planes, domain_num, stride, downsample))
        inplanes = planes * block.expansion
        for i in range(1, blocks):
            self.layers.append(block(inplanes, planes, domain_num))

    def forward(self, x, domain_btcsize=None, use_dsbn=False):
        for m in self.layers:
            x = m(x, domain_btcsize=domain_btcsize, use_dsbn=use_dsbn)
        return x
...
class ResNet(Backbone):
    ...
    def __init__(self, block, layers, **kwargs):
        ...
        self.bn1 = DomainSpecificBN(64, self.domain_num) #nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        ...
        self.layer1 = MakeDomainSpecificLayer(block, 64, 64, layers[0], domain_num=self.domain_num)
        ...
        self.layer2 = MakeDomainSpecificLayer(block, 64, 128, layers[1], stride=2, domain_num=self.domain_num)
        ...
        self.layer3 = MakeDomainSpecificLayer(block, 128, 256, layers[2], stride=2, domain_num=self.domain_num)
        ...
        self.layer4 = MakeDomainSpecificLayer(block, 256, 512, layers[3], stride=2, domain_num=self.domain_num)
    ...
    def featuremaps(self, x, label=None, domain_btcsize=None, use_fmix=True, cam=None, use_dsbn=False):
        ...
        x = self.conv1(x)
        x = self.bn1(x, domain_btcsize=domain_btcsize, use_dsbn=use_dsbn)
        x = self.relu(x)
        x = self.maxpool(x)

        x = self.layer1(x, domain_btcsize=domain_btcsize, use_dsbn=use_dsbn)
        ...
        if self.fmix_cfg[1] == 1 and use_fmix and self.training:
            ...
            x = out_fmix[0]
        x = self.layer2(x, domain_btcsize=domain_btcsize, use_dsbn=use_dsbn)
        if self.fmix_cfg[2] == 1 and use_fmix and self.training:
            ...
            x = out_fmix[0]
        x = self.layer3(x, domain_btcsize=domain_btcsize, use_dsbn=use_dsbn)
        if self.fmix_cfg[3] == 1 and use_fmix and self.training:
            ...
            x = out_fmix[0]
        x = self.layer4(x, domain_btcsize=domain_btcsize, use_dsbn=use_dsbn)
    ...
    def forward(self, x, label=None, domain_btcsize=None, use_fmix=True, cam=None, use_dsbn=False):
...
def init_pretrained_weights(model, model_url):
    ...
    domain_num = model.domain_num
    new_state_dict = pretrain_dict.copy()
    for key, val in pretrain_dict.items():
        if 'layer' in key:
            new_state_dict[key[:7] + 'layers.' + key[7:]] = val.data.clone()

    state_dict = new_state_dict.copy()
    for key, val in new_state_dict.items():
        if 'layer' in key:
            if 'downsample' in key:
                idx = len('layer3.layers.0.downsample.1.')
            else:
                idx = len('layer3.layers.1.bn2.')
        else:
            idx = len('bn1.')
        ...
        if 'bn' in key or 'downsample.1' in key:
            if 'weight' in key:
                for d in range(domain_num):
                    state_dict[key[:idx] + 'bns.{}.weight'.format(d)] = val.data.clone()
            elif 'bias' in key:
                for d in range(domain_num):
                    state_dict[key[:idx] + 'bns.{}.bias'.format(d)] = val.data.clone()
            elif 'running_mean' in key:
                for d in range(domain_num):
                    state_dict[key[:idx] + 'bns.{}.running_mean'.format(d)] = val.data.clone()
            elif 'running_var' in key:
                for d in range(domain_num):
                    state_dict[key[:idx] + 'bns.{}.running_var'.format(d)] = val.data.clone()
            elif 'num_batches_tracked' in key:
                for d in range(domain_num):
                    state_dict[key[:idx] + 'bns.{}.num_batches_tracked'.format(d)] = val.data.clone()
...
def resnet18_fmix_cam_bn(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet34_fmix_cam_bn(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet50_fmix_cam_bn(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet101_fmix_cam_bn(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet152_fmix_cam_bn(pretrained=True, **kwargs):
