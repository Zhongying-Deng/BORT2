...
class CutMix(nn.Module):
    ...
    def forward(self, x, label=None, domain_bs=None, cam=None):
        ...
        output = content_mix_cam(x, self.mix_beta, label, cam=cam)
        x_mix, label1, lam, rand_index, mask, x2, label2, lam2, mask_content2 = output
        #else:
        ...
        return x_mix, label1, lam, rand_index, x2, label2, lam2
...
def content_mix_cam(x, beta, label, cam, domain_bs=None):
    ...
    mask_content1, x1 = copy_paste(mask_mix, x_mix, cam, x)
    ...
    lam1 = 1 - mask_content1[:, 0, :, :].sum(dim=[-2, -1]) / (H * W)
    
    x_mix2 = x[rand_index, :, :, :]
    x_detach = x.clone().detach()
    mask_content2, x2 = copy_paste(cam, x_detach, mask_mix, x_mix)

    lam2 = 1 - mask_content2[:, 0, :, :].sum(dim=[-2, -1]) / (H * W)

    return x1, label1, lam1, rand_index, mask_content1, x2, label, lam2, mask_content2
...
def resnet18_fmix_cam_bidirection(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet34_fmix_cam_bidirection(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet50_fmix_cam_bidirection(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet101_fmix_cam_bidirection(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet152_fmix_cam_bidirection(pretrained=True, **kwargs):
