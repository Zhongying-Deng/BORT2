"""
    Add channel attention (CBAM https://arxiv.org/abs/1807.06521)
    and Adaptive Kernel Module to ResNet backbone
"""
import torch
...
from .dmc_layer import GlobalAdaptKernelModule, GlobalSpatialAdaptKernelModule, AdaptiveKernelModule
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, 
            use_attention=True, output_attention=False):
        super().__init__()
...
class AdapKernelBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_ibn=False, shortcut=False, m=16):
        ...
        self.conv1 = GlobalAdaptKernelModule(3, inplanes, planes, m=m, padding=None, stride=stride)
        #self.conv1 = GlobalSpatialAdaptKernelModule(3, inplanes, planes, m=m, padding=None, stride=stride)
        #self.conv1 = conv3x3(inplanes, planes, stride)
        if use_ibn:
            self.bn1 = IBN(planes)
        else:
...
class ResNet(Backbone):
    ...
    def __init__(self, block, adapblock, layers, **kwargs):
        ...
        self.layer4_adap = self._make_adap_kernel_layer(adapblock, 512, layers[3], stride=2, m=16)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2)
        #self.gate = ChannelGate(2048)
        #self.conv_reduction = nn.Conv2d(4096, 2048, 1)
        #self.bn_reduction = nn.BatchNorm2d(512)
        self.global_avgpool = nn.AdaptiveAvgPool2d(1)
        ...
        for i in range(1, blocks):
            if i == blocks - 1:
                layers.append(block(self.inplanes, planes, 
                    use_attention=use_attention, output_attention=True))
            else:
                layers.append(block(self.inplanes, planes, use_attention=use_attention))

        return nn.Sequential(*layers)

    def _make_adap_kernel_layer(self, block, planes, blocks, stride=1, use_ibn=False, m=16):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, use_ibn=use_ibn, m=m))
        inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(inplanes, planes, use_ibn=use_ibn, m=m))
    ...
    def featuremaps(self, x):
        ...
        x1, out_ca4 = self.layer4(x)
        x2 = self.layer4_adap(x)
        #x = self.gate(x1) + self.gate(x2)
        x = x1 + x2
        #x = torch.cat([x1, x2], 1)
        #x = self.conv_reduction(x)
        #x = self.relu(self.bn_reduction(x))
...
def resnet18_ca_adapt_kernel(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet34_ca_adapt_kernel(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet50_ca_adapt_kernel(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet101_ca_adapt_kernel(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet152_ca_adapt_kernel(pretrained=True, **kwargs):
