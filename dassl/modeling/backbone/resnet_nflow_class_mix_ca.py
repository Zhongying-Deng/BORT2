...
class ResNet(Backbone):
    ...
    def featuremaps(self, x, noise=None):
        ...
        x_all.append(out_ca1)
        x, out_ca2 = self.layer2(x)
        x_all.append(out_ca2)
        x, out_ca3 = self.layer3(x)
        x_all.append(out_ca3)
        x, out_ca4 = self.layer4(x)
        x_all.append(out_ca4)
        return x, x_all
...
def resnet18_nflow_cmix_ca(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet34_nflow_cmix_ca(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet50_nflow_cmix_ca(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet101_nflow_cmix_ca(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet152_nflow_cmix_ca(pretrained=True, **kwargs):
