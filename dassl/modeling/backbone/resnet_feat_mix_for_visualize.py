...
class CutMix(nn.Module):
    ...
    def forward(self, x, label=None, domain_bs=None, cam=None):
        ...
        x_disp = x.clone().detach()
        if r <= self.prob and self.cn_mix and False:
            x = channel_mix(x)#
        x_mix, label1, lam, rand_index, mask = feat_content_mix(x, self.mix_beta, label, cam=cam)
        #else:
        ...
        return x_mix, label1, lam, rand_index, x_disp, mask
...
class ResNet(Backbone):
    ...
    def forward(self, x, label=None, domain_btcsize=None, use_fmix=True, cam=None):
        f, fmix = self.featuremaps(x, label=label,
                domain_btcsize=domain_btcsize, use_fmix=use_fmix, cam=cam)
...
def resnet18_fmix_visualize(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet34_fmix_visualize(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet50_fmix_visualize(pretrained=True, **kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet101_fmix_visualize(pretrained=True, *kwargs):
...
@BACKBONE_REGISTRY.register()
def resnet152_fmix_visualize(pretrained=True, **kwargs):
