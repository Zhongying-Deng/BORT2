...
class FixMatchEMASoftThreshold(FixMatch):
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        with torch.no_grad():
            ...
            #output_u3 = F.softmax(self.teacher(input_u3), 1)
            ...
            #max_prob = 0.5 * (max_prob + max_prob3)

            self.num_samples += mask_u.sum()
            self.total_num += label_u.size()[0]
            #mask = 100 * (1. - float(self.num_samples) / self.total_num) * (max_prob - self.conf_thre)
        ...
        #img_shape = input_u2.shape
        #img_noise = torch.randn(img_shape[0], img_shape[1], img_shape[2], img_shape[3]) * 0.15
        #input_u3 = torch.flip(input_u2, [3]) + img_noise.to(self.device)
        #output_u_noise = self.model(input_u3)
        #output_u = torch.cat([output_u, output_u_noise], 0)
        #label_u = torch.cat([label_u, label_u], 0)
        #mask_u = torch.cat([mask_u, mask_u], 0)
        ...
        #ema_alpha = min(1 - 1 / (global_step+1), 0.8)
