...
class FixMatchLabelMix(FixMatch):
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        with torch.no_grad():
            ...
            values, indices = output_u.detach().topk(1, dim=1, largest=True, sorted=True)
            ...
            self.total_num += label_u_gt.size()[0]

            correct_ps_label = mask_u[start_idx:] * (label_u[start_idx:] == label_u_gt).float()
        ...
        loss_u1 = (loss_u1 * mask_u).mean()
        #if self.epoch > 1:
        low_conf = ~mask_u.bool().unsqueeze(1)
            #low_conf = torch.ones_like(mask_u).unsqueeze(1)
        #else:
        #    low_conf = torch.zeros_like(mask_u).unsqueeze(1)
        warmup = min(1., self.epoch / 4.)
        loss_u_mix = 0
        for i in range(values.size(1)):
            loss_u_mix += values[:, i] * F.cross_entropy(output_u, indices[:, i], reduction='none') #+ \
                         #values[:, 1] * F.cross_entropy(output_u, indices[:, 1], reduction='none')
        loss_u = loss_u1 + (loss_u_mix * low_conf * warmup).mean()
        #loss_u_mix = (loss_u_mix * low_conf).mean()
        ...
        loss_summary = {
            ...
            'loss_u': loss_u.item(),
            #'loss_mix': loss_u_mix.item(),
            'acc_u': compute_accuracy(output_u, label_u)[0].item(),
            'loss_mix': loss_u.item() - loss_u1.item()
        }
    ...
    def test(self):
        ...
        print('among them corrected predictions {}({}/{})'.format(
            float(self.label_ps_quality) / (self.num_samples + 1e-5), self.label_ps_quality, self.num_samples))
        self.num_samples = 0
        self.total_num = 0
        self.label_ps_quality = 0
