...
class FixMatchSFDADistNetMetaLearnRetrain(FixMatch):
    ...
    def __init__(self, cfg):
        ...
        if 'TargetVal' in cfg.DATASET.NAME:
            self.use_val_set = True
            self.target_val_iter = iter(self.val_loader)
    ...
    def build_model(self):
        ...
        if cfg.MODEL.INIT_WEIGHTS:
            init_weights = cfg.MODEL.INIT_WEIGHTS.split('/')
            ckpt = []
            entropy_inverse = []
            for idx, domain in enumerate(source_domains):
                init_weights[2] = domain
                src_model = torch.load('/'.join(init_weights))
                ckpt.append(src_model)
                self.model.load_state_dict(src_model['state_dict'])
                entropy = self.model_entropy(self.model.to(self.device))
                entropy_inverse.append(1. / entropy.cpu().float())
            ent_inv_all = sum(entropy_inverse)
            model_weight = [entropy/ent_inv_all for entropy in entropy_inverse]
            print('fusion weight for each source model {}'.format(model_weight))

            base_model = ckpt[0]['state_dict']
            with torch.no_grad():
                for k, ema_v in base_model.items():
                    model_v = 0.
                    for i in range(len(ckpt)):
                        if i != 0:
                            msd = ckpt[i]['state_dict']
                            model_v += model_weight[i] * msd[k].detach()
                    ema_v.copy_(model_weight[0] * ema_v + model_v)
            
            self.model.load_state_dict(base_model)
        else:
            assert False, 'Pretrained source model is required for source-free DA.'
    ...
    def model_entropy(self, model):
        ...
        for batch_idx in range(num_batches):
            ...
            output = F.softmax(model(input_u), 1)
            ent.append(self.ent(output, dim=1))
        overall_ent = torch.cat(ent, 0).mean()
        return overall_ent.detach()
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        if self.finetune:
            if not self.built_new_optim:
                ...
                if len(cfg.MODEL.INIT_WEIGHTS) == 0:
                    self.new_model.load_state_dict(self.model.state_dict(), False)
                else:
        ...
        if hyper_step and self.finetune:
            ...
            if self.use_val_set:
                input_val, input_val2, _ = self.parse_batch_val()
                output_val = self.new_model(
                    input_val2,
                    use_mixstyle=use_mixstyle,
                    domain_btcsize=None,
                    lmda=None,
                ...
                _, _, _, _, sig_val = output_val
                # with or without F.relu leads to almost the same results on PACS
                entropy_val = F.relu(torch.mean(torch.log(sig_val + 1e-7)))
            else:    
    ...
    def ent(self, output, dim=None):
        entropy = - output * torch.log(output + 1e-6)
        if dim is None:
            return torch.mean(entropy)
        else:
            return torch.mean(entropy, dim=dim)
    ...
    def parse_batch_val(self):
        try:
            batch_val = next(self.target_val_iter)
        except StopIteration:
            self.target_val_iter = iter(self.val_loader)
            batch_val = next(self.target_val_iter)
        input_val = batch_val['img']
        # If this line report a error, modify line 115 of dassl/data/data_manager.py
        # tfm=tfm_train -> tfm=tfm_test
        input_val2 = batch_val['img2']
        label_val = batch_val['label']
        #domain_val = batch_val['domain']
        input_val = input_val.to(self.device)
        input_val2 = input_val2.to(self.device)
        label_val = label_val.to(self.device)
        return input_val, input_val2, label_val
