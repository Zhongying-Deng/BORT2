...
from torch.distributions import MultivariateNormal, Uniform, TransformedDistribution, SigmoidTransform, Beta
...
class FixMatchNFlowClassMixConsistencyDigit5(FixMatch):
    ...
    def build_model(self):
        ...
        self.optim_nflow = build_optimizer(self.nflow_model, cfg.OPTIM, lr_mult=0.01, optim='adam')#4)
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        with torch.no_grad():
            ...
            if self.use_fmix:
                logit, feat, _ = self.model(input_u1, use_fmix=use_fmix,
                        domain_btcsize=domain_btch_size, return_feature=True)
            else:
                logit, feat, _ = self.model(input_u1, return_feature=True)
            ...
            pred_nf = self.nflow_model.predict(feat[bs_src:, :])
        ...
        if self.use_fmix:
            ...
            if sampled_feat is not None:
                output_u, feat, feat_all_u = self.model(input_u2, noise=sampled_feat, return_feature=True)
            else:
                output_u, feat, feat_all_u = self.model(input_u2, noise=None, return_feature=True)
    ...
    def test(self):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            if self.use_fmix:
                output, feat = self.model_inference(input)
            else:
                output, feat = self.model_inference(input)
            feat_u = feat[-1]
        ...
        print('Until epoch {}, best accuracy of student model {}, NFlow model {}, enssemble {}'.format(self.epoch, max(self.acc), max(self.acc_teacher), max(self.acc_ens)))
