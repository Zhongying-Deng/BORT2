...
class Digit5Predictor(nn.Module):
    def __init__(self):
        super(Digit5Predictor, self).__init__()
...
class DomainBasedNet(SimpleNet):
    ...
    def __init__(self, cfg, model_cfg, num_classes, **kwargs):
        ...
        if 'resnet' in model_name:
            self.classifier = Predictor(fdim, num_classes)
        elif 'digit5' in model_name:
            self.classifier = Digit5Predictor()
...
class FixMatchTSVDDistNetMetaLearnRetrain(FixMatch):
    ...
    def build_model(self):
        ...
        if cfg.MODEL.INIT_WEIGHTS:
            try:
                state = torch.load(cfg.MODEL.INIT_WEIGHTS)
                self.model.backbone.load_state_dict(state['G'], strict=False)
                self.model.classifier.load_state_dict(state['C'], strict=False)
            except:
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        if self.finetune:
            if not self.built_new_optim:
                ...
                self.register_model('model_meta_final', self.new_model, 
        ...
        if hyper_step and self.finetune:
            self.model.train()
            pred_u = F.softmax(self.model(input_u1, use_mixstyle=False,
                                            domain_btcsize=None, lmda=None), 1)
            max_prob, _ = pred_u.max(1)
