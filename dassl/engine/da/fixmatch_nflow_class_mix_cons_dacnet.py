...
class FixMatchNFlowClassMixConsDACNet(FixMatch):
    ...
    def __init__(self, cfg):
        ...
        self.weight_cons = cfg.TRAINER.NFLOW.WEIGHT_CON
        ...
        self.weight_cls = cfg.TRAINER.CALOSS.WEIGHT_CON
        self.weight_d = cfg.TRAINER.CALOSS.WEIGHT_D  # domain attention consistency loss weight
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        if sampled_feat is not None:
            ...
            sampled_feat_src = None
        if self.use_fmix:
            output_x, src_ca = self.model(input_x, use_fmix=False, noise=sampled_feat_src)
        else:
            output_x, src_ca = self.model(input_x, noise=sampled_feat_src)
        ...
        if self.weight_d > 0:
            ...
            tar_ca1 = feat_all_u[-2] # get target attention weights, index is -2
            ...
            tar_ca2 = feat_all_u[-3] # get target attention weights
            ...
            src_ca_tmp = torch.split(src_ca[-3], self.split_batch, 0)
            for ind, ca in enumerate(src_ca_tmp):
                domain_idx = domain_x[ind]
        ...
        if self.weight_cls > 0:
            with torch.no_grad():
                ...
                output_u3 = F.softmax(feat_u3, 1)
                max_prob3, label_u3 = output_u3.max(1)
                mask_u3 = (max_prob3 >= self.conf_thre).float()
                label_consist = (label_u[bs_src:] == label_u3).float()
                mask_u = mask_u[bs_src:] * mask_u3 * label_consist

            if mask_u.sum() > 0:
                weight = self.model.classifier.weight
                feat_u = feat_all_u[-1][bs_src:]
                diff = mask_u.unsqueeze(1).expand_as(feat_u) * (feat_u - weight[label_u3, :])
                loss_comp = self.weight_cls * torch.mean(diff * diff)
            else:
