...
class FixMatchMSCMOneStep(FixMatch):
    ...
    def __init__(self, cfg):
        ...
        if self.ent_thre:
            ...
            self.thre_param = 1. #cfg.TRAINER.FIXMATCH.CONF_THRE
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        critic_lmda = 2 / (1 + np.exp(-10 * progress)) - 1
        ...
        with torch.no_grad():
            ...
            prob_u, _ = output_u[start_idx:].max(1)
        ...
        output_u, feat = self.model(input_u2, use_mixstyle=use_mixstyle,
                          domain_btcsize=domain_btch_size, lmda=lmda, return_feature=True)
        del domain_btch_size[-1]
        # domain discriminator
        feat_x, feat_u = torch.split(feat, [input_x2.size(0), input_u.size(0)])
        feat_x = self.revgrad(feat_x, grad_scaling=critic_lmda)
        feat_u = self.revgrad(feat_u, grad_scaling=critic_lmda)
        ...
        with torch.no_grad():
            ratio_tmp = torch.sigmoid(output_xd).detach()
            ratio_tmp = torch.min(ratio_tmp, torch.tensor(critic_lmda).to(self.device))
            ratio = ratio_tmp.unsqueeze(2).unsqueeze(3).expand_as(input_x)
            x_mix_weight = 1. - ratio_tmp
        if not self.finetune:
            output_x = self.model(input_x)
            loss_x = torch.mean(x_mix_weight * F.cross_entropy(output_x, label_x, reduction='none'))
        else:
            use_mixstyle = False
        ...
        loss_summary = {
            ...
            'x_mix_max': x_mix_weight.max().item(),
            'x_mix_min': x_mix_weight.min().item(),
            'x_mix_mean': x_mix_weight.mean().item(),
            'loss_d': loss_d.item()
        }

        if not self.finetune:
            loss = loss_x + (loss_u + loss_u_mix) * self.weight_u + 0.001 * loss_d
