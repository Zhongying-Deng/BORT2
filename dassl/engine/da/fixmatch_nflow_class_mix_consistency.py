...
class FixMatchNFlowClassMixConsistency(FixMatch):
    ...
    def build_model(self):
        ...
        if 'VisDA17' in cfg.DATASET.NAME:
            self.optim_nflow = build_optimizer(self.nflow_model, cfg.OPTIM, lr_mult=0.1, optim='adamw')
            self.f_cache_len = 10
        else:
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        with torch.no_grad():
            ...
            mask_agreed = (label_u[bs_src:] == label_u_nflow)
            mask_disagreed = (~mask_agreed)
        ...
        if self.epoch < 2 * self.warmup:
            ...
            if mask_agreed.sum() > 0:
                loss_cons = weight_cons * domain_discrepancy(pred_nf[mask_agreed], pred_c[mask_agreed], 'L2')
            else:
                loss_cons = torch.tensor(0.)
            if torch.isnan(loss_cons):
                print('GDC loss is NAN. weight_cons: {}'.format(weight_cons))
