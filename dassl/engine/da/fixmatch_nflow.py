...
class FixMatchNFlow(FixMatch):
    ...
    def build_model(self):
        ...
        dim = 128
        # construct the normalizing flow model
        self.nflow_model = NFlowList(dim, self.total_domain)
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        feat = intermediate_feat[1]#[0]
        loss_all = self.nflow_model(feat.detach(), domain_label=domain_x, domain_bs=domain_btch_size)
        ...
        for i in range(len(domain_x)):
            ...
            sampled_feat.append(sampled_noise[-1].clone().detach())
        ...
        if self.use_fmix:
            output_u, fmix = self.model(input_u2, use_fmix=use_fmix,
                                  domain_btcsize=domain_btch_size, label=label_u, noise=sampled_feat[::-1])
            ...
            output_u, feat_all_u = self.model(input_u2, noise=sampled_feat[::-1])
