...
class FixMatchMSCMDistNetMetaLearnOnestep(FixMatch):
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        if self.finetune:
            if not self.built_new_optim:
                ...
                self.optim_new_model = build_optimizer(self.new_model, self.cfg.OPTIM, lr_mult=1)
            ...
            self.model.eval()
        if self.finetune:
            if ((global_step+1) % self.meta_optim_step == 0) and (self.epoch >= self.max_epoch / 2.):
        ...
        input_u1_final_model = input_u
        input_u2_final_model = input_u2
        start_idx = 0
        ...
        with torch.no_grad():
            ...
            if self.ent_thre:
                ...
                self.conf_thre2 = self.thre_param

            max_prob, label_u = output_u.max(1)
            mask_u = (max_prob >= self.conf_thre).float()
            mask_u_final_model = (max_prob[start_idx:] >= self.conf_thre2).float()
            ...
            r = np.random.rand(1)
            if r < self.cutmix_prob and not hyper_step:
        ...
        if not hyper_step:
            ...
            loss_summary = {
                ...
                'threshold': self.conf_thre2
            }
            
        use_mixstyle = False
        output = self.new_model(
                input_u2_final_model, 
        ...
        for logit in output_u_sample:
            loss_s = F.cross_entropy(logit, label_u[start_idx:], reduction='none')
            loss_sampled += (loss_s * mask_u_final_model).mean()
        loss_sampled /= len(output_u_sample)

        loss += loss_sampled + self.weight_ent * loss_entropy #+ consistency_loss
