...
class DomainBasedNet(SimpleNet):
    ...
    def forward(self, x, return_feature=False, domain_btcsize=None,
                use_fmix=False, label=None, noise_d=None, noise_c=None):
        if self.use_fmix:
            f = self.backbone(x, domain_btcsize=domain_btcsize,
                    use_fmix=use_fmix, label=label)
        else:
            f = self.backbone(x, noise_d=noise_d, noise_c=noise_c)
...
class NFlowListDomain(nn.Module):
    ...
    def forward(self, x, domain_label=None, domain_bs=None):
...
class NFlowListClass(nn.Module):
    def __init__(self, dim, n_classes):
        ...
        self.feat_cache = [[0.] for _ in range(n_classes)]
        self.initial_done = [False for _ in range(n_classes)]
        self.cache_len = [32 for _ in range(n_classes)]
        nflow_models = []
        for _ in range(n_classes):
            ...
            norms = [ActNorm(dim=dim) for _ in flows]
            flows = list(itertools.chain(*zip(norms, convs, flows)))
            nflow_models.append(NormalizingFlowModel(prior, flows))
        # construct the normalizing flow model
        self.nflow_list = nn.ModuleList(nflow_models)

    def forward(self, x, label=None, domain_bs=None, mask=None):
        # class-wise modeling
        loss = []
...
class FixMatchNFlowDomainClassMix(FixMatch):
    ...
    def build_model(self):
        ...
        dim = 128
        self.nflow_model = NFlowListDomain(dim, self.total_domain)
        self.nflow_model.to(self.device)
        print('# params of domain-wise NFlow model: {:,}'.format(count_num_param(self.nflow_model)))
        ...
        self.register_model('nflow_model_d', self.nflow_model, self.optim_nflow, self.sched_nflow)
        dim = 256
        self.nflow_model_c = NFlowListClass(dim, self.num_classes)
        self.nflow_model_c.to(self.device)
        print('# params of class-wise NFlow model: {:,}'.format(count_num_param(self.nflow_model_c)))
        self.optim_nflow_c = build_optimizer(self.nflow_model_c, cfg.OPTIM, lr_mult=1.)
        self.sched_nflow_c = build_lr_scheduler(self.optim_nflow_c, cfg.OPTIM)
        self.register_model('nflow_model_c', self.nflow_model_c, self.optim_nflow_c, self.sched_nflow_c)
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        # forward normalizing domain-wise flow model
        feat = intermediate_feat[1].clone().detach()
        loss_all = self.nflow_model(feat, domain_label=domain_x, domain_bs=domain_btch_size)
        loss_flow = self.weight_nflow * sum(loss_all) / len(loss_all)
        self.model_backward_and_update(loss_flow, names='nflow_model_d')
        sampled_feat_d = []
        for i in range(len(domain_x)):
            ...
            sampled_feat_d.append(sampled_noise[-1].clone().detach())
        
        feat_c = intermediate_feat[2].clone().detach()
        loss_c = self.nflow_model_c(feat_c, label=label_u, 
                          domain_bs=domain_btch_size,
                          mask=mask_u)
        loss_flow_c = self.weight_nflow * sum(loss_c) / len(loss_c)
        if loss_flow_c != 0:
            self.model_backward_and_update(loss_flow_c, names='nflow_model_c')
        else:
            self.model_zero_grad(names='nflow_model_c')

        sampled_feat_c = []
        if self.epoch < self.warmup:
            sampled_f_d = sampled_feat_d[::-1] #None
            sampled_f_c = None
        else:
            sampled_f_d = sampled_feat_d[::-1]
            for i in range(input_u2.size(0)):
                # class wise augmentation
                d_idx = label_u[i]
                sampled_noise = self.nflow_model_c.nflow_list[d_idx].sample(1)
                sampled_feat_c.append(sampled_noise[-1].clone().detach())
        ...
        if self.use_fmix:
            ...
            output_u, feat_all_u = self.model(input_u2, 
                                     noise_d=sampled_f_d,
                                     noise_c=sampled_f_c)
        ...
        loss_summary = {
            ...
            'loss_flow_d': loss_flow.item(),
            'loss_flow_c': loss_flow_c.item()
        }
