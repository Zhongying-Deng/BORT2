...
class FixMatchNFlowClassMixSelfConsBaseline(FixMatch):
    ...
    def __init__(self, cfg):
        ...
        self.self_consistency = True
        if self.self_consistency:
            print('Use the prediction from discriminative classifier to supervised itself (||weak_aug_pred - strong_aug_pred||_2^2).')
        else:
            print('Use the disagreed prediction from generative classifier to supervised the discriminative one (||weak_aug_pred - strong_aug_pred||_2^2).')
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        with torch.no_grad():
            ...
            pred_weak_aug = output_u.clone().detach()
            ...
            if self.epoch >= 2*self.warmup:
                mask_u_disagree = (label_u_nflow != label_u[bs_src:,]).float()
                self.num_disagree += mask_u_disagree.sum()
    ...
    def test(self):
        ...
        print('samples above the threshold {}({}/{}), disagreed samples {}'.format(
            float(self.num_samples ) / self.total_num, self.num_samples, self.total_num,
            float(self.num_disagree) / self.total_num))
