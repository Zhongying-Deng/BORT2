...
class FixMatchMSCMDistNetRetrain(FixMatch):
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        if self.finetune:
            if not self.built_new_optim:
                ...
                self.new_model.load_state_dict(model_weights, strict=False)
            ...
            if self.finetune:
                ...
                if self.ent_thre:
                    ...
                    if self.batch_idx == 0 and (
                        ...
                        self.thre_param = torch.clamp(prob_u.mean() + prob_u.std(), 0., self.thre_param)
        ...
        if not self.finetune:
            ...
            self.ema_model_update(self.new_model, self.model)
    ...
    def ema_model_update(self, model, ema, decay=0.999):
