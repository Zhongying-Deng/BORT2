...
class Invertible1x1Conv(nn.Module):
    ...
    def _assemble_W(self):
        ...
        tmp = torch.diag(torch.ones(self.dim))
        #if self.L.is_cuda:
        #    tmp = tmp.cuda()
        #    self.P = self.P.cuda()
        tmp = tmp.to(self.L.device)
        self.P = self.P.to(self.L.device)
        L = torch.tril(self.L, diagonal=-1) + tmp
...
class NormalizingFlowModel(nn.Module):
    ...
    def forward(self, x):
        ...
        prior_logprob = self.prior.log_prob(zs[-1]+1e-5).view(x.size(0), -1).sum(1)
