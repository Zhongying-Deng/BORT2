...
class NSF_AR(nn.Module):
    ...
    def __init__(self, dim, K=5, B=3, hidden_dim=8, base_network=MLP):
        super().__init__()
        self.dim = dim
        self.K = K  # K decide the output dim of self.layers
        self.B = B
        self.layers = nn.ModuleList()
        self.init_param = nn.Parameter(torch.Tensor(3 * K - 1))
        for i in range(1, dim):
            self.layers += [base_network(i, 3 * K - 1, hidden_dim)]  # each element in x is dependent on the previous elements， i.e. (x_i | x_{<i})
