...
# class SSLGaussMixture(torch.nn.Module, torch.distributions.Distribution):
class SSLGaussMixture(torch.distributions.Distribution):
    def __init__(self, means, inv_cov_stds=None, weights=None, param_learnable=False, device=None):
        # torch.nn.Module.__init__(self) 
        # torch.distributions.Distribution.__init__(self)  
    ...
    def sample(self, sample_shape, gaussian_id=None):
        if gaussian_id is not None:
            ...
            idx = np.random.choice(self.n_components, size=(n_samples, 1), p=F.softmax(self.weights).cpu().numpy())
