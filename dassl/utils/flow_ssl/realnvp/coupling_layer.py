...
class CouplingLayerTabular(CouplingLayerBase):
    ...
    def __init__(self, in_dim, mid_dim, num_layers, mask, init_zeros=False,dropout=False):
        ...
        #last_layer = nn.Linear(mid_dim, in_dim)
        if init_zeros:
        ...
        self.rescale = RescaleTabular(in_dim)
        #self.rescale = RescaleTabularWeightNorm(in_dim)
...
class RescaleTabularWeightNorm(nn.Module):
    def __init__(self, D):
        super(RescaleTabularWeightNorm, self).__init__()
        self.weight = nn.Parameter(torch.ones(D))

    def forward(self, x):
        weight_direction = self.weight / self.weight.norm(dim=0, keepdim=True)
        weight_magnitude = self.weight.norm(dim=0, keepdim=True)
        x = weight_direction * x * weight_magnitude
        return x
